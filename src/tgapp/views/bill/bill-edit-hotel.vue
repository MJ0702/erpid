  ...
  <div>
    <header class="mint-header is-fixed">
      ...
      <div class="center-button single-over">
        <span class="header-title single-over middle-span">现付-房</span>        
      </div>
    </header>    
    <div style="height:40px"></div>
    <div><hotelComponent></hotelComponent></div>    
  ...
  export default {
    ...
    methods: {    
        confirm(){
          ...
          let model = this;
          this.$store.dispatch('tgapp/hotel/confirm',  function(res){
                if(res.success){
                    Toast('保存成功！'); 
                    router.history.go(-1);
                    //清空导游现付输入的表单数据
        ...
        deleteRecode(){
          let router = this.$router; 
          let model = this;  
          this.$store.dispatch('tgapp/hotel/delete', function(res){
               if(res.success){
                    Toast('删除成功！');  
                    router.history.go(-1);
                    //清空导游现付输入的表单数据
