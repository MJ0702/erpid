<template>
  <div id="shopping_testclass">
    <!--金额输入-->
    <div class="shopping_toolbar_box">
      <div class="shopping_price_toolbar">
        <div class="left_box">
          <div class="shopping_currency_box">
            <span class="shopping_currency">{{ currencyType }}</span><br>
            <span class="shopping_currency_code">USD</span>
            <img class="shopping_down_icon" src="assets/down.svg" @click="currency_box_show">
          </div>
        </div>
        <div class="middle_box">
            <input class="shopping_price_in_input" type="number" placeholder="0.00" :disabled="isDisabled" v-model="input_content" @input="countTotalMoney">
          <div class="shopping_clear_price">
              <img src="assets/close.svg" @click="clear_input_content">
          </div>
          <span class="shopping_dashed_line"></span>
        </div>
        <div class="right_box">
          <div class="shopping_add_time" @click="open('addtime_picker')" size="large" :startDate="startDate">{{addTimeValue}}
    ...
    <!--显示汇率-->
    <div class="shopping_price_rate">
      <div class="shopping_total_money">
        <span>人民币：</span><span>{{total_money}}</span>
      </div>
      <div class="shopping_split_line"></div>
      <div class="shopping_rate_box">
        <span>汇率</span>
        <b>{{rate}}</b>
      </div>
    </div>
    ...
    <div class="shopping_info_box">
      <div class="shopping_info_content">
        <div class="shopping_choose_box">
            <div class="shopping_info_title">购物</div>
            <div class="shopping_info_choose_input">
                <input type="text" placeholder="请选择购物店" @focus="search_shopping">
                <div>
                    <img src="assets/right.svg">
                </div>
            </div>
        </div>
        <div class="shopping_remark">
            <div class="shopping_remark_title">摘要</div>
            <div class="shopping_remark_input">
    ...
    <!--搜索框-->
    <div class="shopping_mask" v-show="search_mask_box">
        <div class="search_top_box">
           <input type="text" :class=" isFocus ? 'shopping_search_input' : 'focus_shopping_search_input' " placeholder="搜索" @focus="search_focus">
...
.shopping_toolbar_box{
    width: 100%;
    height:66px;
    background-color:#fff;
    border-bottom:1px dashed #F3F7FC;
}
.shopping_price_toolbar{
    width:100%;
    height:35px;
    padding:15px 0;
    background-color:#fff;
...
.shopping_clear_price>img{
    width:12px;
    height:12px;
}
.shopping_price_in_input{
    height:35px;
    line-height: 35px;
    width: 75%;
    height: 35px;
...
.shopping_total_money,.shopping_rate_box{
    padding:10px 0;
    margin-left: 15px;
    line-height: 15px;
}
.shopping_total_money>span,.shopping_rate_box>span{
    font-size: 12px;
    color: #ccc;
}
.shopping_split_line{
...
.shopping_shopping_box{
    width: 100%;
}
.shopping_info_content{
    height:104px;
    background: #FFF;
    margin: 0 10px;    
}
.shopping_shopping_content{
    height: 100px;
    background: #FFF;
    margin: 0 10px;
    position: relative;
...
.shopping_type_choose{
    margin-left: 10px;
    width: 52%;
}
.shopping_choose_box,.shopping_remark{
    margin: 0 15px;
    height: 50px;
    display: flex;
    flex-flow: row;
...
.shopping_info_title,.shopping_remark_title{
    font-size: 14px;
    margin-right: 20px;
    color: #606266;
}
.shopping_info_choose_input{
    flex:1;
    display: flex;
    margin-right: 5px;
}
.shopping_info_choose_input>input{
    width: 60%;
    border: none;
}
.shopping_info_choose_input>input::-webkit-input-placeholder{ /*WebKit browsers*/
    color:#B0B4B3;
}
.shopping_info_choose_input>input::-moz-input-placeholder{ /*Mozilla Firefox*/
    color:#B0B4B3;
}
.shopping_info_choose_input>input::-ms-input-placeholder{ /*Internet Explorer*/ 
    color:#B0B4B3;
}
.shopping_remark_input{
    flex: 1;
    border: none;
}
.shopping_remark_input>input{
    border: none;
}
.shopping_remark_input>input::-webkit-input-placeholder{ /*WebKit browsers*/
    color:#C0C4CC;
}
.shopping_remark_input>input::-moz-input-placeholder{ /*Mozilla Firefox*/
    color:#C0C4CC;
}
.shopping_remark_input>input::-ms-input-placeholder{ /*Internet Explorer*/ 
    color:#C0C4CC;
}
.shopping_down_list{
    text-align: center;
...
.shopping_down_list_icon{
    height: 20px;
}
.shopping_down_list_icon>img{
    width: 18px;
    height: 18px;
}
.shopping_down_list_tip{
    color: #A4A8AE;
}
.form_info_box{
    padding:0 0 10px 0;
