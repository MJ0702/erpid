<template>
  <div class="order-place-block">
    ...
    <my-block></my-block>
    <mt-cell title="客户公司" is-link :value="customer.simName" @click.native="$router.push({name:'company-search'})"></mt-cell>
    ...
    <mt-field label="QQ" placeholder="请输入客户QQ" :attr="{maxlength:30}" type="number" v-model="customer.qq"></mt-field>
...
export default {
  ...
  computed: {
    customer() {
      return this.$store.getters["customer/customer_new"];
    },
  },
  methods: {
    addCustomer() {
      if(!this.customer.trueName) {Toast("请输入客户姓名");return;}
      if(!this.customer.cpyID||!this.customer.cpyID_text) {Toast("请选择客户公司");return;}
      if(!this.customer.mobile){Toast("请输入客户手机号");return;}
      Indicator.open({
        text: '正在操作...',
        spinnerType: 'fading-circle'
      });
      this.$store.dispatch("customer/customer_add").then(state => {
        Indicator.close();
        if(state.success) {
          Toast("保存成功");
          //请求客户详情数据
          this.$store.dispatch("customer/customer_request", state.userID);
