<template>
  <div>
    <mt-header title="团期订单" fixed>
      <router-link :to="routerTo[type]" slot="left">
        <mt-button icon="back">返回</mt-button>
      </router-link>
      <mt-button slot="right" @click="$router.push({path: '/plan-order-search'})">筛选</mt-button>
    </mt-header>
  ...
  >-->
    ...
    <div class="my-preview" v-for="order in orders" @click="orderClick(order)">
      ...
      <div class="my-preview_row1">
        <div class="my-preview_row1_left">订单：{{order.ordID}}
          <!--<img src="~assets/order-list-sale.png" class="order-list-icon">-->
        </div>
        <!--<div class="my-preview_row1_middle">订单：{{order.ordID}}</div>-->
      ...
      <mt-cell-swipe :right="(PermitCheck('me-receipt')&&order.isOK>=0)?[
        ...
        <div class="my-preview_row2">
          <div class="my-preview_row2_top">
            ...
            <span v-if="userCpyNameShow">客户：{{order.userCpyName}}</span>
            <span v-else></span>
  ...
  export default {
    data() {
      return {
        ...
        orderQueryAction: ["order/plan_orders_request", "order/plan_orders_search"],
        routerTo: ["/line-plan", "/line-plan"],
        userCpyNameShow: true,
      }
    },
    computed: {
      orders() {
    ...
    created() {
      ...
      if (this.sys.con_sysShow_20) {
        let userCpyNameShow = this.sys.con_sysShow_20.split(',')[6];
        if (userCpyNameShow == 0) this.userCpyNameShow = false
      }
    ...
    methods: {
      orderClick(order) {
        if (order.saleID!=this.user.ID) {
          Toast('非自己的订单，无权访问订单详情');
          return;
        }
        this.$router.push({
          name: "order-info",
