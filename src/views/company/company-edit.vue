<template>
  <div class="order-place-block">
    ...
    <mt-cell title="结算价格" is-link @click.native="jiesuanShow(2)" class="require-field" v-model="price"></mt-cell>
    ...
    <mt-cell title="所在地区" is-link class="input-right" @click.native="addressShow=true" >
      <input type="text" max="50" @click.stop.prevent v-model="company.proveinceArea"  placeholder="请选择或输入" class="mint-field-core">
    </mt-cell>
  ...
  export default {
    ...
    created(){
      if(!this.company.ID){
        ...
        let priceType=['市场价','同行价','成本价'];
        this.price=priceType[this.company.isVip];
      }
    ...
    methods: {
      ...
      Vue.http.post(
        ...
        sort:'[{"property":"bySort","direction":"desc"},{"property":"isLock","direction":"asc"},{"property":"ID","direction":"desc"}]'
          ...
          let data = res.body.rows;
          let priceType=['市场价','同行价','成本价'];
          this.price=priceType[data[0].isVip];
      ...
      openAddress(arg) {
        ...
        if (arg == 'qr') {
          ...
          this.company.proveinceArea = this.addressProvince +' '+this.addressCity;
      ...
      addcompany() {
        ...
        Vue.http.post(
          ...
          if (data.success) {
            ...
            this.$router.push({name:'company-info',query:{id:ID}})
          } else {
            Toast(data.msg);
          }
        }).catch(err => {
