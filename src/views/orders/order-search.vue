<template>
  <div class="order-search">
    ...
    <my-cell title="订单来源" class="newline" :value="query.sources" is-link @click.native="sourcesClick"></my-cell>
    <mt-cell title="占位">
    ...
    <mt-header :title="['','订单状态','日期类型','订单来源'][showWhich]" fixed v-if="popupStatus" style="z-index:2100">
    ...
    <mt-popup v-model="popupStatus" popup-transition="popup-fade" position="right" class="my-popup-right">
      ...
      <mt-checklist v-model="sourcesSelect" :options="sources" @change="sourcesChange" v-if="showWhich==3"></mt-checklist>
...
<script>
  export default {
    ...
    created() {
      this.$store.dispatch("order/sources_request").then(end => {
        if (this.query.sources) {
          let sources = this.query.sources.split(',');
          this.sourcesSelect = sources;
        }
      });
    },
    computed: {
      query() {
        return this.$store.getters["order/orders_search_query"];
      },
      sources() {
        return this.$store.getters["order/sources"];
      }
    },
    methods: {
      sourcesClick() {
      ...
      searchOrder() {
        this.$router.push({ path: "/order-search-list" });
      },
      ...
      sourcesChange(val) {
        let sources = "";
        sources = val.map(v => v);
        this.query.sources = sources.join();
      }
    },
    watch: {
    },
