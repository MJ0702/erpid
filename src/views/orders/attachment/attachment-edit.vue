<template>
  ...
  <div>
    ...
    <mt-field class="require-field input-right"
              label="文件名称"
              placeholder="请输入文件名称"
              :value="attachmentSelect.title"></mt-field>
    <my-block></my-block>
    <my-cell title="上传水单"
             :value="attachmentSelect.files"></my-cell>
...
export default {
  ...
  computed: {
    attachmentSelect() {
      let select = this.$store.getters["order/attachment/attachmentSelect"];
      if (select.files) {
        let fileImg = {};
        if (/[' ']/g.test(select.files)) {
          fileImg.img = decodeURI(select.files);
        } else {
          fileImg.img = select.files
        }
  ...
  methods: {
    ...
    fnEdit() {
      ...
      this.$store.dispatch("order/attachment/attachment_edit", data);
      this.$router.go(-1);
    },
